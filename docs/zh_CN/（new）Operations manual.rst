...
```yaml
title = "flato global configurations"
commercialVersion = "2.0"
##########################################################
#
# Logger section
...
[audit]
	backend				= "file" # "file" or "filelog" or graylog" or "elk"
 	level         		= "none" # none, metadata, responsebody, storageobject
    queue_size          = 4096
    rewrite_interval    = "2s"
    update_conn_interval = "2s"
	[audit.conn.pool]
#    	urls             = ["172.16.3.1:12201", "172.16.3.2:12201", "172.16.3.3:12201", "172.16.3.4:12201"]  #graylog tcp conn urls
    	urls             = ["172.16.5.5:12202"]    #filebeat tcp conn url
        init_cap         = 1
        max_cap          = 5
        idle_timeout     = "60s"
        dial_timeout     = "3s"
    [audit.file]               #filelog config
        encrypt = false
        type="filelog"
        [audit.file.filelog]
            path = "system/auditlog/filelog/"
            ...
            index_enable = true
            [audit.file.filelog.cache]
                enable = true
                max_cache_size = 100 #mb
                cache_expired_time = 48 #hour
                cache_entry_num = 20

            [audit.file.filelog.handler_cache]
...
http_2   = false
#Path to tlsca for rpc
tlsCA                   = "./tls/tlsca.ca"
#Path to tlscert for rpc
tlsCert                 = "./tls/tls_peer.cert"
#Path to private key of tlscert for rpc
tlsCertPriv             = "./tls/tls_peer.priv"
    [http.request]
    max_content_length = "100kb" # default 100kB
    read_timeout = "1000s"      # default 1000s
...
[consensus]
    [consensus.set]
    # set_size       = 25    # How many transactions should the node broadcast at once
    set_timeout    = "0.1s"# Node broadcasts transactions if there are cached transactions, although set_size isn't reached yet

    [consensus.pool]
    # batch_size       = 500    # How many txs should the primary pack before sending pre-prepare
    # pool_size        = 50000  # How many txs could the txPool stores in total
    check_interval  = "3m"  # interval of the check loop
    tolerance_time  = "5m"  # the max tolerance time, if time.Now - timestamp > toleranceTime, send event to consensus
    batch_mem_limit  = false # If Limit the batch memory size or not
    batch_max_mem    = "10mb" # The maximum memory size of one batch

    [consensus.rbft]         # rbft configurations
    k                = 10    # After how many blocks a checkpoint will be sent
    vc_period        = 0     # After how many checkpoint periods( Blocks = 10 * vcperiod ) the primary gets cycled automatically. ( Set 0 to disable )
        ...
        [consensus.rbft.timeout]
            sync_state        = "1s"  # How long to wait quorum sync state response
            sync_interval     = "10s"  # How long to restart sync state process
            recovery          = "10s" # How long to wait before recovery finished
            ...
            fetch_checkpoint  = "5s"  # How long to wait for config change checkpoint quorum before fetch checkpoint take

    [consensus.raft]        # RAFT configurations
    snap_count        = 20 # How many entries should trigger a snapshot
    catchup_count = 20 # when doing log compaction, keep some entries in memory for slow followers to catchup

        [consensus.raft.timeout]
            batch        = "0.5s"  # Node make a batch if there are pending requests, although batchsize isn't reached yet
            set          = "0.1s" # Node broadcasts transactions if there are cached transactions, although set_size isn't reached yet
            fetch  = "3s" # How long to fetch a missing batch
            negotiate = "6s" # How long to wait for quorum responses after send negotiate

        [consensus.raft.dir]
            snap = "data/raft/snap" # snapshot dir
            wal  = "data/raft/wal"  # wal dir
...
[encryption]
[encryption.TEE]
TEEPath = "http://nexus.hyperchain.cn/repository/arch/sgx/enclave.sign.so"
DataEncrypt = false
...
[encryption.bcert]
 # path of the back-up permission certs
 listDir = "certs/bcerts"
...
[distributedCA]
enable = false  #true is aco mode

[encryption.security]
algo   = "sm4"   # Selective symmetric encryption algorithm (pure,3des,aes or sm4)
...
[self-government.upgrade]
archive_path = "upgrade_archive"
archive_prefix = "upgrade_"
fetch_remote = true
fetch_path = "http://127.0.0.1:8000/hyperchain-deploy.tar.gz"

[[self-government.nodes]]
hostname    = "node1"
address     = "14f14e4e316ed13c41eedd4759d79d07aac775ac"

[[self-government.nodes]]
hostname    = "node2"
address     = "916a3c39f7c0f579c9df5b9931af49a899c5bb4a"

[[self-government.nodes]]
hostname    = "node3"
address     = "be79466f637b4c18de25f0312c5961af66fb2d3d"

[[self-government.nodes]]
hostname    = "node4"
address     = "e593460a7c31f9984deae6529a57229285bd0370"
...
[duplicate]
# tx generated in the past tx_active_time is legal
tx_active_time    = "24h"
# The transaction's timestamp can only be greater than the current time by up to tx_drift_time
tx_drift_time     = "5m"

[duplicate.bloomfilter] # bloom filter used in tx duplication checking
# each bloomfilter's size
bloombit          = 100000000
# Bloom filter's total memory limit
max_mem  = "100mb"
...
urls				= ["localhost:9092"]
writerBatchSize		= 100
writerBatchBytes	= 52428800
writerBatchTimeout 	= "0.1s"
writeTimeout 		= "10s"
...
[executor.nvp]
exitflag = false
sync_mode = "block" #another mode is journal #新增

[executor.sync_chain]
priority = "block"
sync_journal_receipt = true
hs_interval = "5s"
hs_resend_times = 5
batch_size = 100
state_sync_interval = "2s"
fetcher_resend = 5
mode_negotiate_interval = "5s"

[executor.filemgr]
enable = false
deadline = "600s"
clean_interval = "60s"
file_system_mode = "ipfs" #origin, ipfs
...
```text
# 创建test空目录
mkdir test && cp rockit test && cd test
# 初始化目录
rockit dir
```
...
[tx]
    ...
    # if use for flato, please use 2.2 to replace default
    version = "2.2"
...
```text
crete proposal and init node account

Usage:
  rockit init [flags]

Examples:
rockit init --ns global --nodePub "hello1 hello2 hello3 hello4" --nodes  "node1 node2 node3 node4"

Flags:
      --admins string     grant admin role to given account, split by space. for example:0x00000000000001 0x11111111
  -h, --help              help for init
      --nodes string      init hostname of nodes,default node1-node4 (default "node1 node2 node3 node4")
      --nodesPub string   public key of nodes, in publickey file, input public key file name, defaule node1-node4 (default "node1 node2 node3 node4")
      --threshold int     proposal threshold, default 1 (default 1)

Global Flags:
      --ns string   ns of nodes,default global (default "global")
...
```text
# 使用默认值初始化，即节点名为node1-node4，不初始化新的admin账户，提案投票阈值初始化为1,namespace为global
rockit init
# 使用指定的值初始化
#rockit init --nodes "node1 node2 node3 node4 node5" --publickey "node1 node2 node3 node4 node5" --admins "0x9202d80df4c6d658290bc0c18fc2ddeb08735c8c  0x0eb804bf69adb78d19555db1f869f26ccc2c0cfb" --threshold 2
```
...
Flags:
      --cfca             not use decentralized ca,default false
  -h, --help             help for addvp
      --node string      hostname of new node,default node5 (default "node5")
      --nodePub string   public key's absolute path of new node (default "node5")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011wfa45bbf0eb3389efacf9a92428f0541f?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXdmYTQ1YmJmMGViMzM4OWVmYWNmOWE5MjQyOGYwNTQxZiJ9.kl5M0MplicthwHyeZ9F9QcycIPzk5vtz9alnZv9mk8o&download=image.png "")
...
Examples:
rockit rmvp --node node5 --ns global

Flags:
  -h, --help          help for rmvp
      --node string   name of the deleted node ,default node5 (default "node5")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011w98b5f4fbe7c8b33701ab0a923c6133e7?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXc5OGI1ZjRmYmU3YzhiMzM3MDFhYjBhOTIzYzYxMzNlNyJ9.xO8keUpladKNZThtpOT3lAGSfdd_2cU9L_RbyXuoTQU&download=image.png "")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011w261361a218517e97245093302b5df49d?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXcyNjEzNjFhMjE4NTE3ZTk3MjQ1MDkzMzAyYjVkZjQ5ZCJ9.deTIxvPOUCM45UEjc_wYOPY1egDOIoBEycVJtGKnjWQ&download=image.png "")

### 6.1.3 检测通信延迟

network ping <hostname>：测试到某个节点的网络延迟。

![](http://teambitiondoc.hyperchain.cn:8099/storage/011w08a72dd56ac7e7d7bb0d79913fd43d05?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXcwOGE3MmRkNTZhYzdlN2Q3YmIwZDc5OTEzZmQ0M2QwNSJ9.niC96q4VwhzQeZb3A2emxDCe43imR6rQ6vwmi6ZqrIo&download=image.png "")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011w8b910bb11f54b6e5d1cce1418425741b?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXc4YjkxMGJiMTFmNTRiNmU1ZDFjY2UxNDE4NDI1NzQxYiJ9.apwweTQ72WXSLWTzyZ36PihCv0_WMUxxJ38_SGLhDkQ&download=image.png "")

## 6.3 LICENSE信息查询

通过ipc命令可以获取到本机ip，LICENSE绑定ip以及LICENSE的过期时间。

![](http://teambitiondoc.hyperchain.cn:8099/storage/011ha88966b120af3cd1b178e8635bd3eee3?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMWhhODg5NjZiMTIwYWYzY2QxYjE3OGU4NjM1YmQzZWVlMyJ9.aJY87FVRQJCvUdQQxpkYvXs2T24-4aw69Xvox6w3zbM "")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011w5c0cb6a3b7b533344ef2fa3e6c200d77?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXc1YzBjYjZhM2I3YjUzMzM0NGVmMmZhM2U2YzIwMGQ3NyJ9.SFDp4rxxGWHdWdQDTciLk75YUahkOmPWStBCoVShtV4&download=image.png "")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011wb870a24174374fd2b6da9d3b29cbf2aa?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXdiODcwYTI0MTc0Mzc0ZmQyYjZkYTlkM2IyOWNiZjJhYSJ9.W03xxhkHnJpCRoB-1nx89x3d9bXr7VOn_dpVVC0264c&download=image.png "")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011w141e4605f7dde4769c4fd103eb9c746c?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMXcxNDFlNDYwNWY3ZGRlNDc2OWM0ZmQxMDNlYjljNzQ2YyJ9._1ls691OdlWCQRmYIU2taxtXtT76yPgt3N6f4UVYpuo&download=image.png "")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/0120543570430738709a9242f5443510b4b2?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMjA1NDM1NzA0MzA3Mzg3MDlhOTI0MmY1NDQzNTEwYjRiMiJ9.MiOQw46PmCuc8b3NHbIHXRJ3CRZ3fVGxC0DsHumw-mQ&download=image.png "")
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/0120b64821793fe931405b6736d5a962e992?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTYxNDU4OTE0OSwiaWF0IjoxNjEzOTg0MzQ5LCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMjBiNjQ4MjE3OTNmZTkzMTQwNWI2NzM2ZDVhOTYyZTk5MiJ9.ad4HZnb7y6WIFy2NOwTZ59G14Q32rULkk3vzCqLF-v0&download=image.png "")
